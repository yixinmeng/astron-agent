...
> **状态**：v1+v2 审核已吸收、v3 清稿已处理、**v4 自核已落地**（2026-04-17）
> **作者**：yixinmeng
> **最后更新**：2026-04-17（v4 自核版——RAGFlow Server 源码 checkout v0.20.5/v0.24.0 对比）
...
> - **v4（本次自核，颠覆性结论）**：拉 RAGFlow Server 源码 + checkout `v0.20.5` / `v0.24.0` 实锤对比——**`highlight` 不是幻觉**（v2 误判，Server 层实际存在，SDK 未封装）；**v0.20.5 已支持 6 个高级字段**（use_kg / cross_languages / metadata_condition / highlight / rerank_id / keyword）；**D1 决策从"倾向升级"翻转为"倾向保守"**（v0.20.5 → v0.24.0 实际只多 `toc_enhance` 1 个字段 + 1 处 `highlight` bug 修）。详见 §9。
...
- **描述**：仓库锁定的 RAGFlow server 镜像和 SDK 版本与官方最新版存在代差。v4 实测证明**能力开放层面的差异极小**——仅 1 个字段 + 1 处 bug 修。
- **证据**：
...
- **v4 实测差异（checkout v0.20.5 vs v0.24.0 对比 `api/apps/sdk/doc.py`）**：
  ...
  | 维度 | v0.20.5 | v0.24.0 | 差异 |
  |------|---------|---------|------|
  | `/retrieval` 支持字段 | use_kg / cross_languages / metadata_condition / highlight / rerank_id / keyword / top_k / similarity_threshold / vector_similarity_weight（9 个） | 上述 + **toc_enhance**（10 个） | **仅 +1 字段** |
  | `highlight` 判定 | 字符串比较 `== "False"` | `req.get("highlight", None)` | bug 修 |
  | 分页参数 | page/size（Server 层） | page/page_size | 参数名微调 |

- **影响（v4 重估）**：
  - RF-08 **不再被 RF-18 阻塞**——v0.20.5 已经支持 6 个高级字段，立刻可开放
  - RF-10 的 chunk_method 集合差异需要单独核实（见 RF-10 章节）
  - 能力开放 PR 基本可在当前基线 v0.20.5 上推进
- **改动面与决策建议（v4 重写）**：
  - **推荐保守锁 v0.20.5**（当前生产已验证）：
    - 升级 ROI 极低：仅多 1 个 `toc_enhance` 字段 + `highlight` bug 修
    - 升级风险：跨 4 个 minor release 的完整兼容性测试、镜像 tag 命名变化、SDK 升级的 breaking change
  - **若开放 `highlight`**：需绕过 v0.20.5 的字符串比较 bug（传 `str(bool_value)` 或通过 env flag 仅在 >= v0.24.0 时开放）
  - **激进升级**（不推荐，除非有 `toc_enhance` 的强需求）：同步 `docker/ragflow/.env` + `pyproject.toml` + 完整回归
