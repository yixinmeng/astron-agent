...
> **状态**：Codex 独立审核 v1 + v2 已吸收；v3 清稿审核已处理（2026-04-17）
> **作者**：yixinmeng
> **最后更新**：2026-04-17（Codex v2 反馈修订版 + v3 清稿）
...
- **描述（Codex v1 修正 + v2 清稿）**：RAGFlow retrieval API 支持若干高级参数（rerank / keyword 混合检索 / GraphRAG 等），但 astron-agent 未暴露。**具体可开放字段集以 RF-18 锁定的 SDK 版本为准**。
- **证据**：
  ...
  - `core/knowledge/domain/entity/chunk_dto.py:136-151` `ChunkQueryReq`
    > schema 无对应字段
- **可开放字段（Codex v2 已下载 SDK 0.13.0 / 0.24.0 源码包独立核实）**：
  - ✅ SDK 0.24.0 源码确认支持：`rerank_id`、`keyword`、`use_kg`（GraphRAG）、`similarity_threshold`（已用）、`vector_similarity_weight`（已用但硬编码，见 RF-09）
  - ❌ **删除 v1 遗留幻觉字段**：`question_history` / `document_ids_for_reranking` / `highlight` 在 SDK 0.13.0 和 0.24.0 源码包中**均未找到**。其中 `highlight` 是 Codex v2 第二次揪出的幻觉，务必不要基于记忆新增字段
  - ⚠️ RAGFlow Server HTTP API 层是否另有不通过 SDK 暴露的参数（如 Server 端支持 `highlight` 但 SDK 未封装），本次未核实，需开专门 issue 对齐
...
- **证据**：
  ...
  - **已知 breaking change（Codex v2 核实）**：自 **v0.22.0 起**，官方 Docker 镜像**不再带 `-slim` 后缀**，只发布 slim edition。仓库当前 pin 的 `v0.20.5-slim` tag 在升级路径上需要重命名。
- **行为**：仓库基线与官方主线存在较大版本代差（v0.20.5 → v0.24.0，跨 4 个 minor release）；这之间的 breaking change 和新增能力都不可知。
- **影响**：
  - RF-08（暴露高级检索参数）的字段集完全由 SDK 版本决定
  - RF-10（chunk_method 集合）同理
  - 任何能力开放类 PR 若不先锁版本，都可能上游合并后因版本差异而炸生产
- **改动面**：
  - 先决定是"保守对齐当前基线 v0.20.5"还是"升级基线到 v0.24.x"
  - 若升级：需提交 `docker/ragflow/.env` + `pyproject.toml` + 各配置文件的同步 PR，并做兼容性回归；注意 v0.22.0 起镜像 tag 命名已变
- **PR 友好度**：⭐⭐（需与讯飞沟通基线策略）
- **验证命令**：
  ```bash
...
**D1 阻塞的是"版本敏感的能力开放类 PR"，不是所有 PR**。以下项不依赖 RF-18 版本基线，可并行或先推：

- **不依赖 RF-18**：RF-07（JSON split documentId）、RF-19（page_size 截断）、RF-21（配置不一致）、RF-11（SDK 重构，若选择保守基线）
- **依赖 RF-18**：RF-08 / RF-10（字段集需 SDK 版本确认）、RF-18 自身

### 立即动手建议（Codex v2 双档建议）

**档位 A：深度集成主线（推荐）—— 第一个 PR 做 D1**

原因：RF-08 / RF-10 的字段集、RF-11 的 SDK 重构、RF-20 的测试基线都依赖版本决策。不锁基线先推能力开放 PR，合并后可能因版本差异产生连锁 breaking change。

**D1 具体动作**：
1. 查 `docker/ragflow/.env` 确认当前默认镜像版本（已确认：v0.20.5-slim）
2. 跑 `python -c "import ragflow_sdk; print(ragflow_sdk.__version__)"` 确认 SDK 版本（已确认：0.13.0）
3. 核对 RAGFlow v0.20.5 → **v0.24.0**（当前 latest）的 changelog / breaking changes
   - 已知 breaking：v0.22.0 起镜像不再带 `-slim` 后缀
4. 决定：**保守对齐现基线 v0.20.5** 还是 **升级到 v0.24.x**
5. 把决策结论回写本文档作为 D1 结论

**档位 B：低风险代码 PR 试水**

如果想先拿一个低风险 PR 建立与上游的协作默契（而不立即触发版本决策），从以下三选一（都不依赖 D1）：

- **RF-07**：JSON `/document/split` 端点支持 `documentId`，补全 PR #1181 的重切去重覆盖面
- **RF-19**：`page_size=1000` 截断——改为按 id 直查或真分页遍历
- **RF-21**：`RAGFLOW_DEFAULT_GROUP` fallback 统一到 `RagflowUtils.get_default_group()`

**第二个 PR（D2 启动）：RF-01 跨层改造**

- **不是**纯 Python 修复（Codex v1 指正）
- **Java 侧缺口精确位置**：`KnowledgeService` + `KnowledgeV2ServiceCallHandler`（Codex v2 精确定位）
...
- [ ] 阅读 RAGFlow v0.20.5 → **v0.24.0**（当前 latest，2026-02-10）的 CHANGELOG / release notes
- [ ] 留意已知 breaking change：v0.22.0 起 Docker 镜像不再带 `-slim` 后缀
- [ ] 决策：保守对齐 v0.20.5 / 激进升级 **v0.24.x**
- [ ] 把决策结论回写至 RF-18 章节
...
| `highlight` 在 SDK 0.13.0/0.24.0 源码中均无 | RF-08 | 删除 `highlight`，标注"v2 已下源码核实" |
...
- **ragflow-sdk 0.24.0 未见**：`highlight`（需开 issue 向 RAGFlow 确认 Server HTTP API 层是否暴露）

### 8.5 v2 对 D 路线的修正

- **D1 不阻塞所有 PR**：只阻塞**版本敏感的能力开放类 PR**（RF-08 / RF-10 等）
- **RF-07 / RF-19 / RF-21 可先行**：与版本基线无关，是纯代码质量问题
- **RF-11 （SDK 重构）** 依赖 D1 决策——如果保守在 0.13.0，重构意义有限；如果升级 0.24.0，重构大幅增值
- **RF-03 时机**：v2 不建议提前到 RF-01 之前；最多提前到"RF-01 落地后立即做"
- **第一个 PR 双档建议**：
  - 深度主线：D1（版本基线决策）
  - 低风险试水：RF-07 / RF-19 / RF-21 三选一
...
**RF-08 已连续两轮翻车**：
- v1：编造 `question_history` / `document_ids_for_reranking`
- v2：保留的 `highlight` 同样未核实

**规则**：未来涉及外部 SDK / 第三方 API 字段，**必须先下载源码包 grep 验证**，不能靠记忆或类比。这是 Codex v1 和 v2 两次用实际 diff 教会我的教训。
